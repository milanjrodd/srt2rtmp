...
const gstPath = import.meta.env.GST_PATH;

if (!rtmpUrl || !srtUrl || !gstPath) {
  throw new Error(
    "RTMP_URL, SRT_URL and GST_PATH environment variables are required."
  );
}

await $`${gstPath} srtsrc uri=${srtUrl} blocksize=${blocksize} mode="caller" auto-reconnect=false ! tsdemux \
